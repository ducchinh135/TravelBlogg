...
function renderPosts(posts) {
    ...
    let htmls = posts.map(function (post) {
        return `
        <post-contenttravel class="post-content" id="${post.id}" image="${post.image}" commentLength="${post.comment.length}" title="${post.title}" mainHead="${post.mainHead}" ></post-contenttravel>
        <hr>`
    });
